...
class UpdateWeather extends Command
{
     ...
     * @var string
     */
    protected $signature = 'update-weather:cron';
    ...
    public function handle()
    {
        ...
        foreach ($districts as $district) {
            ...
            $w_api = json_decode(Weather::forecast($district['name']), true);

            if ($w_api['cod'] == 200) {
                $weather = Weather::updateOrCreate([
                    'location' => $w_api['name'],
                ], [
                    'longitude' => $w_api['coord']['lon'],
                    'latitude' => $w_api['coord']['lat'],
                    'humadity' => $w_api['main']['humidity'],
                    'temperature' => round($w_api['main']['temp'] - 273.15, 2),
                ]);
                ...
                // if any error
                $msg = 'Cod ' . $w_api['cod'] . '. Message : ' . $district['name'] . ' ' . $w_api['message'];
                logger()->error($msg);
            }
